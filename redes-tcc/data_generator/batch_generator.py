...
class BatchGenerator:
    ...
    def __init__(self,
        ...
        Arguments:
            box_output_format (list, optional): A list of strings representing the desired order of the
                items in the generated data. If it is a classification problem, there should be only a 'class_ID' string. 
                If it is a recognition problem, the list should have class ID, xmin, ymin, xmax, ymax. 
                If you want to train a SSD model, the expected strings must be the order that the box encoding class
                requires as input. In this case, defaults to `['class_id', 'xmin', 'ymin', 'xmax', 'ymax']`. 
                Note that even though the parser methods are able to produce different output formats, the SSDBoxEncoder 
                currently requires the format `['class_id', 'xmin', 'ymin', 'xmax', 'ymax']`. This list only specifies 
                the five box parameters that are relevant as training targets, a list of filenames is generated separately.
            task (string, optional): Can be either `classification` or `recognition`. If it is `classification`, 
                the box output format list wont have any object bounding box, and if it is recognition, the class
                will consider multiple bounding boxes for an image output.
                ...
                if not annotations_dir is None:
                    ...
                    for obj in objects:
                        ...
                        if self.task == 'recognition':
                            ...
                            raise ValueError("`task` can be either 'classification' or 'recognition'.")
                        box = []
    ...
    def generate(self,
        ...
        Arguments:
            ...
            ssd_train (bool, optional): Whether or not the generator is used in training mode. If `True`, then the labels
        ...
        if shuffle:  # Shuffle the data before we begin
            if (self.labels is None) and (self.image_ids is None):
                ...
                self.filenames, self.labels, self.image_ids = sklearn.utils.shuffle(self.filenames, self.labels,
                                                                                    self.image_ids)
        current = 0
        ...
        if self.task == 'recognition':
            ...
            ios = np.amin(
                [xmin, ymin, xmax, ymax])  # Index offset, we need this for the inverse coordinate transform indices.
        ...
        while True:
            ...
            batch_inverse_coord_transform = np.array([[[0, 1]] * 4] * batch_size,
            ...
            for i in range(len(batch_X)):
                ...
                if flip:  # Performs flips along the vertical axis only (i.e. horizontal flips).
                    ...
                    if p >= (1 - flip):
                        ...
                        if self.task == 'recognition' and not ((batch_y is None) or (len(batch_y[i]) == 0)):
                ...
                if scale:
                    ...
                    if p >= (1 - scale[2]):
                        ...
                        if self.task == 'recognition' and not ((batch_y is None) or (len(batch_y[i]) == 0)):
                            ...
                            toplefts = np.array(
                                [batch_y[i][:, xmin], batch_y[i][:, ymin], np.ones(batch_y[i].shape[0])])
            ...
            if ssd_train:  # During training we need the encoded labels instead of the format that `batch_y` has
                ...
                if 'matched_anchors' in returns:
                    batch_y_true, batch_matched_anchors = ssd_box_encoder.encode_y(batch_y,
                                                                                   diagnostics=True)  # Encode the labels into the `y_true` tensor that the SSD loss function needs.
                else:
